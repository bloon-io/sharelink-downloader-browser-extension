// BSDBE means Bloon Sharelink Downloader Browser Extension
window.BSDBE_RemoteTreeDataManager =
    class RemoteTreeDataManager {
        ...
        async retrieveCurrentRemoteTreeData() {
            ...
            const socket = new WebSocket(RemoteTreeDataManager.BLOON_ADJ_API_WSS_URL);
            try {
                ...
                const api = new BSDBE_WssApiCaller(socket);
        ...
        async _getChildFolderRecursiveUnit(api, shareID, bloonID, folderIDs, localRelPath, treeData) {
            ...
            for (const folder of folderDatas) {
                ...
                const folderName = BSDBE_Utils.getAvailableFileBaseName(name, true);
                let childCards = [];
                let childRelPath = "";
                ...
                if (!localRelPath) {
                    ...
                    while (childRelPath in treeData.folder_set) {
                        ...
                        index += 1;
                        childRelPath = localRelPath + "/" + BSDBE_Utils.getFileName(folderName, '', index);
        ...
        _handle_childFiles(localRelPath, childCards, treeData) {
            for (const childCard of childCards) {
                ...
                const chC_name = BSDBE_Utils.getAvailableFileBaseName(childCard.name, !chC_extension);
                ...
                let index = 0;
                let chC_localRelPath = localRelPath + "/" + BSDBE_Utils.getFileName(chC_name, chC_extension, index);
                while (chC_localRelPath in treeData.file_dict) {
                    console.log("file path exist: " + chC_localRelPath);
                    index++;
                    chC_localRelPath = localRelPath + "/" + BSDBE_Utils.getFileName(chC_name, chC_extension, index);
